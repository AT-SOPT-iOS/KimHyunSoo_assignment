...
struct RealtimeLiveView: View {
    let liveList: [RealtimeLiveModel] = RealtimeLiveModel.dummy()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 12) {
                    ForEach(liveList, id: \.id) { item in
                        RealtimeLiveCell(data: item)
                    }
                }
...
struct RealtimeLiveCell: View {
    let data: RealtimeLiveModel
